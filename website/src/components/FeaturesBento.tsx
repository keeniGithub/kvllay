import { Terminal, Shield, Clock, Lock, Box, Monitor } from "lucide-react"
...
export function FeaturesBento() {
  ...
  return (
    <section id="features" className="w-full flex flex-col items-center py-16 px-6 lg:px-10 max-w-[1240px] mx-auto scroll-mt-20">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {features.map((item) => {
          ...
          return (
            <div
              key={item.title}
              className="bg-[#0E131F] border border-[#1B2436] hover:border-[#1E3B5C] rounded-xl p-6 flex flex-col justify-start gap-3.5 transition-colors shadow-sm group"
            >
              ...
              <h3 className="text-[17px] font-bold text-[#F0F6FC]">{item.title}</h3>
